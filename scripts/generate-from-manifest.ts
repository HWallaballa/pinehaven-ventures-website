...
function generateSeedStripe(): string {
  ...
  lines.push(`  if (!dryRun) {`);
  lines.push(`    const productPriceMap: Record<string, { prices: string[]; intervals: string[] }> = {};`);
  lines.push(`    let idx = 0;`);
  lines.push(`    for (const pd of products) {`);
  lines.push(`      const prices: string[] = [];`);
  lines.push(`      const intervals: string[] = [];`);
  lines.push(`      for (let i = 0; i < pd.plans.length; i++) { prices.push(allPriceIds[idx]); intervals.push(pd.plans[i].interval); idx++; }`);
  ...
  lines.push(`        productPriceMap[prodId] = { prices, intervals };`);
  lines.push(`      }`);
  lines.push(`    }`);
  lines.push(`    // Stripe portal requires unique billing intervals per product.`);
  lines.push(`    // Only include products where all prices have distinct intervals.`);
  lines.push(`    const portalProducts = Object.entries(productPriceMap)`);
  lines.push(`      .filter(([, { intervals }]) => new Set(intervals).size === intervals.length)`);
  lines.push(`      .map(([p, { prices }]) => ({ product: p, prices }));`);
  lines.push(`    const hasSubscriptionUpdate = portalProducts.length > 0;`);
  ...
  lines.push(`        subscription_update: hasSubscriptionUpdate ? {`);
  lines.push(`          enabled: true, default_allowed_updates: ['price'],`);
  lines.push(`          proration_behavior: 'create_prorations',`);
  lines.push(`          products: portalProducts,`);
  lines.push(`        } : { enabled: false },`);
  lines.push(`      },`);
  lines.push(`    });`);
  lines.push(`    console.log('  Portal configured' + (hasSubscriptionUpdate ? '' : ' (plan switching disabled — products have duplicate intervals)'));`);
